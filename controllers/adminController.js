    ...
        // 1. Tìm người dùng chính
        const user = await User.findById(userId).lean();
        if (!user) {
          return res.status(404).send('Người dùng không tồn tại');
        }

        // 2. Lấy danh sách tài khoản phụ (Đã sửa từ 'parent' thành 'created_by')
        const subUsers = await User.find({ created_by: userId }).lean();

        // 3. Lấy danh sách trẻ do người dùng tạo (Đã sửa từ 'createdBy' thành 'user_id')
        const children = await Child.find({ user_id: userId }).lean();

        // 4. Lấy danh sách nhắc nhở do người dùng tạo hoặc liên quan đến trẻ (Đã sửa 'createdBy' thành 'user_id' và 'child' thành 'child_id')
    ...
        // 5. Bài viết do người dùng tạo (Đã sửa từ 'createdBy' thành 'id_user')
        const posts = await Post.find({ id_user: userId }).lean();

        // ✅ Trả về view `user-details.ejs`
